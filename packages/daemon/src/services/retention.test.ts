...
function makeRepos({
  ...
  return { configRepo, tickMetricsRepo, decisionsRepo, alertsRepo };
}

describe('RetentionService', () => {
  it('invokes each prune with the correct cutoff from config', async () => {
    const now = 1_700_000_000_000;
    const { configRepo, tickMetricsRepo, decisionsRepo, alertsRepo } = makeRepos();
    ...
    expect(alertsRepo.pruneOlderThan).toHaveBeenCalledWith(now - 30 * DAY_MS);
  });
  ...
  it('skips a prune when its retention is 0 (keep forever)', async () => {
    ...
    expect(alertsRepo.pruneOlderThan).not.toHaveBeenCalled();
  ...
  it('survives an individual prune throwing (keeps pruning the other tables)', async () => {
    ...
    const svc = new RetentionService(
      ...
      decisionsRepo,
      alertsRepo,
      { now: () => 1_700_000_000_000, log: (m) => logs.push(m) },
    );
    ...
    expect(alertsRepo.pruneOlderThan).toHaveBeenCalled();
  });
  ...
  it('returns zeros without crashing when config row is missing', async () => {
    ...
    expect(r.alerts_deleted).toBe(0);
    expect(tickMetricsRepo.pruneOlderThan).not.toHaveBeenCalled();
    expect(alertsRepo.pruneOlderThan).not.toHaveBeenCalled();
  });
});
