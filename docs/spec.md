...
- Cumulative block reward income detected at `btc_payout_address` via Electrs or `bitcoind` RPC, stored in sat
  (`reward_events.value_sat`; there is no per-event receipt-time USD valuation - USD display uses the current
  oracle price, and historical USD context comes from per-tick `btc_usd_price` in `tick_metrics`). Two modes (#170, toggle
