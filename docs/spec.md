...
| 2.8     | 2026-05-27 | Profit overlay on the Price chart (#220). New `profit / bucket` right-axis option renders signed bars per chart bucket: emerald above zero for profit, rose below for loss, hatched fill on the trailing in-progress bucket. Math computed daemon-side in `withProfit()` on the /api/metrics route: `profit_sat = (Δocean_unpaid_sat + Δpaid_total_sat) - Δprimary_bid_consumed_sat` over consecutive aggregated rows. Bid-swap buckets (negative cost delta) and pre-migration buckets render as gaps not zero bars. Right-axis range spans zero so the baseline is always in view. Hover bar opens a tooltip with bucket time range, revenue / cost / net profit broken out in the global denomination. No new schema; reuses existing tick_metrics columns. |